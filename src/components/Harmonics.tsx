...
const Harmonics = ({
  ...
  const handleColumnClick = (
    ...
    else if (params.field == "Va") a[3] = !a[3];
    else if (params.field == "Vb") a[4] = !a[4];
    else if (params.field == "Vc") a[5] = !a[5];
  ...
  useEffect(() => {
    ...
    if (dataIndex - secondaryIndex >= N && N > 0) {
      ...
      harmonicService
        .getAllHarmonics(selectedFile, secondaryIndex, dataIndex)
        .then((res) => {
...
function HarmonicsTable({
  ...
  if (selectedSignals === undefined)
    mySelectedSignals = [true, false, false, false, false, false];
  ...
  for (let i = 0; i < dataset.length; i++) {
    if (valueType == "percent") {
      ...
      signal[3] = `${Math.round((dataset[i].va / dataset[1].va) * 100).toFixed(
        0
      )}%`;
      signal[4] = `${Math.round((dataset[i].vb / dataset[1].vb) * 100).toFixed(
        0
      )}%`;
      signal[5] = `${Math.round((dataset[i].vc / dataset[1].vc) * 100).toFixed(
        0
      )}%`;
    } else {
      ...
      signal[3] = (Math.round(dataset[i].va * 100) / 100).toFixed(2);
      signal[4] = (Math.round(dataset[i].vb * 100) / 100).toFixed(2);
      signal[5] = (Math.round(dataset[i].vc * 100) / 100).toFixed(2);
    }
    rows.push({
      id: i,
